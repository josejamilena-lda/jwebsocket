		...
		if (Ext.form.Basic) {
			...
			Ext.form.Basic.prototype.submit = function (aOptions) {
				return this.doAction(this.standardSubmit ? 'standardsubmit' :
						this.api ? 'directsubmit' : (this.jwsSubmit ||
								(this.owner && this.owner.jwsSubmit)) ? 'jwssubmit' :
						'submit', aOptions);
			};
			//:m:*:Ext.form.load
			...
			Ext.form.Basic.prototype.load = function (aOptions) {
				return this.doAction(this.api ? 'directload' : (this.jwsSubmit ||
						(this.owner && this.owner.jwsSubmit)) ? 'jwsload' : 'load', aOptions);
			};
		}
	},
	//:m:*:open
			...
			if (typeof this.fTokenClient.isConnected === "function" && !this.fTokenClient.isConnected()) {
				...
				this.fTokenClient.open(lUrl, {
					...
					OnClose: function (aToken) {
						...
						self.fireEvent('OnClose', aToken);
					},
					OnTimeout: function () {
						self.fireEvent('OnTimeout');
					},

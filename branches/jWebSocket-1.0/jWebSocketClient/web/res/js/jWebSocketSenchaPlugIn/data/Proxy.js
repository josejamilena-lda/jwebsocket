...
Ext.define('Ext.jws.data.Proxy', {
	...
	doRequest: function (aOperation, aCallback, aScope) {
		...
		if (typeof lFtokenClient.isConnected !== "undefined" && lFtokenClient.isConnected()) {
			...
			Ext.jws.Client.on("OnOpen", function () {
				lExecutionScope(aOperation, aCallback, aScope, self);
			});
		}
	},
	isEmpty: function (aObject) {
		for (var lIdx in aObject) {
			if (aObject.hasOwnProperty(lIdx)) {
				return false;
			}
		}
		return true;
	...
	setupDataForRequest: function (aRequest) {
		...
		lData = Ext.apply({}, aRequest.rawData, lJsonData, lXMLData) || null;
		...
		lData = !this.isEmpty(lData) ? lData : {};
		if(!lParams){
			lParams = {};
		}
		lParams = Ext.apply(lParams, lData);
		return {
			ns: lNS,
			type: lTokenType,
			data: lParams

...
Ext.ns("admin");
admin.mName = "jWebSocket Management Desk";
admin.mVersion = admin.mName + " 1.0";
admin.NS_SYSTEM = "org.jwebsocket.plugins.system";
admin.NS = "org.jwebsocket.plugins.admin";

function renderResult(aValue) {
	if (aValue === "Successful" || true === aValue) {
		return '<span style="color:green;">' + aValue + '</span>';
	} else if (aValue === "Warning") {
		return '<span style="color:orange;">' + aValue + '</span>';
	} else {
		return '<span style="color:red;">' + aValue + '</span>';
	}
	return aValue;
}

function newGridPanel(aTitle, aStore) {
...
function newWindowAdd(type, storeJar, storeId, store) {
	...
	Ext.getCmp('comboId' + type).on('select', function (cmb, record, index) {
		...
		Ext.jwsClient.send(admin.NS, "get" + type + "ConfigById", {
			id: this.getValue()
		}, {
		...
		items: [{
				...
				items: [{
						...
						handler: function () {
							...
							if ("" != lReason) {
								Ext.jwsClient.send(admin.NS, aFunction, {
									id: aId,
									reason: lReason
								}, {
									success: function (aToken) {
...
function msg(type, message, delay, align) {
	...
	function createBox(t, s) {
		if (!s)
			s = "Debe entrar un texto para el mensaje";
	...
	if (!align)
		align = "t";
	if (!msgCt)
		msgCt = Ext.core.DomHelper.insertBefore(document.body, {
			id: 'msg-div'
		}, true);
			...
			items: [{
					...
					handler: function logon() {
						if (("" != Ext.getCmp('textUser').getValue()) && ("" != Ext.getCmp('textPass').getValue())) {
							Ext.jwsClient.send(admin.NS_SYSTEM, "logon", {
...
admin.mainPanel = {
	init: function () {
			...
			bbar: {
				items: ['-', {
						...
						handler: function changeOrder() {
							...
							if (view.getSelectionModel().hasSelection()) {
								Ext.jwsClient.send(admin.NS, fn, {
						...
						handler: function logout() {
							Ext.jwsClient.send(admin.NS_SYSTEM, "logoff", {}, {
								success: function (aToken) {
		...
		plugin.processToken = function (aToken) {
			if (aToken.ns == admin.NS && (aToken.type == 'processChangeOfPlugIn' || aToken.type == 'processChangeOfFilter')) {
		...
		Ext.jwsClient.addPlugIn(plugin);
	},
	close: function () {
...
admin.logsPanel = {
	init: function () {
		...
		lPlugIn.processToken = function (aToken) {
			if (aToken.ns === admin.NS && aToken.type === 'traceLog') {
				var lLog = new Log({
	...
	init: function () {
		Ext.create('Ext.window.Window', {
			...
			items: [{
					...
					xtype: 'label',
					//text:'Username:',
					html: "<b>" + admin.mName + '</b> is the administrator toolbox of jWebSocket, licensed under the GNU Lesser General Public License. For more information, please visit <a href="http://www.jwebsocket.org" target="_blank">www.jwebsocket.org</a>.',
					width: 340,
					cls: "textAbout",
					x: 10,
					y: 170
					...
					xtype: 'label',
					html: "<b>Version: </b>" + admin.mVersion,
					cls: "textAbout",
					x: 10,
					y: 240
				}, {
...
Ext.onReady(function () {
	Ext.jwsClient.on('OnOpen', function () {
		Ext.QuickTips.init();
		Ext.jwsClient.send(admin.NS_SYSTEM, "getAuthorities", {}, {
			success: function (aToken) {
				if (0 == aToken.authorities.indexOf("ROLE_ADMIN_MAINTENANCE")) {
					admin.mainPanel.init();
					admin.logsPanel.init();

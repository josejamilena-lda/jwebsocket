...
$.widget("jws.presenter", {
	...
	stopAutoSlide: function(){
		w.presenter.isPaused = true;
		clearTimeout(w.presenter.mRepeatTimeout);
	},
	startResumeAutoSlide: function (aDirection) {
		w.presenter.stopAutoSlide();
		w.presenter.isPaused = false;
		var lCondition = function () {
			return !w.presenter.isPaused;
		};
		w.presenter.repeatWhile(lCondition, 3000, function () {
			if ("left" === aDirection) {
				w.presenter.mCurrSlide--;
				// rotating
				if (0 > w.presenter.mCurrSlide) {
					w.presenter.mCurrSlide = w.presenter.mMaxSlides;
				}
			} else {
				w.presenter.mCurrSlide++;
				if (w.presenter.mCurrSlide >= w.presenter.mMaxSlides) {
					w.presenter.mCurrSlide = 0;
				}
			}
	...
	repeatWhile: function (aCondition, aMsec, aCallback) {
		var lCallMyself = function () {
			w.presenter.repeatWhile(aCondition, aMsec, aCallback);
		};
		while (aCondition()) {
			if (typeof aCallback === "function") {
				aCallback();
			}
			w.presenter.mRepeatTimeout = setTimeout(lCallMyself, aMsec);
			return;
		}
	},
